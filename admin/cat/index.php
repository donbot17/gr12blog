...
if (@isset($_POST['sortDesc'])) {
    if ($_SESSION['asc'] == true) {
        $order = "ORDER BY categories.descr ASC";
        $_SESSION['asc'] = false;
    } else {
        $order = "ORDER BY categories.descr DESC";
        $_SESSION['asc'] = true;
    }
}
if (@isset($_POST['sortPub'])) {
    if ($_SESSION['asc'] == true) {
        $order = "ORDER BY categories.viewed ASC";
        $_SESSION['asc'] = false;
    } else {
        $order = "ORDER BY categories.viewed DESC";
        $_SESSION['asc'] = true;
    }
}
...
@$query = "SELECT * FROM categories WHERE category LIKE '%$search%' OR descr LIKE '%$search%' $order";
...
while ($row = mysqli_fetch_array($result)) {
    if (isset($_GET['search'])) {
        ...
        $row['descr'] = preg_replace('/(' . $search . ')/i', '<mark>$1</mark>', $row['descr']);
        $row['cat_id'] = preg_replace('/(' . $search . ')/i', '<mark>$1</mark>', $row['cat_id']);
    }
    $res[] = $row;
}
...
<body>
    ...
    <div class="main-wrapper">
        ...
        <section class="cta-section theme-bg-light py-5">
            <div class="container text-center">
                <h2 class="heading"><a href="../">Manage Catagories</a></h2>
                ...
                <table class="table table-striped my-5">
                    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                        <tr>
                            ...
                            <th scope="col"><button name=sortDesc style='background-color: transparent; border-color:transparent;color:#6f42c1'><b>Category Description</b></button></th>
                            <th scope="col"></th>
                        </tr>
                    </form>
                    <?php
    ...
    <td ><a class='link' href='edit.php?id=$id&redirect='>Edit</a></td>
                ...
                <a href="create.php" class="btn btn-primary">Create a Catagory</a>
                <!-- <a href = 'create.php'> Create a catagory </a> -->
